...
namespace DotEditor.GUIExtension.Windows
{
    ...
    {
        public delegate void OnSelectionCallback(UnityObject obj);

        System.Type mType;
        string mTitle;
        OnSelectionCallback mCallback;
        UnityObject[] mObjects;
        ...
        static public void Draw<T>(string buttonName, T obj, OnSelectionCallback cb, bool editButton, params GUILayoutOption[] options) where T : UnityObject
        {
            GUILayout.BeginHorizontal();
            bool show = GUILayout.Button(buttonName, "DropDown", GUILayout.Width(76f));
        ...
        static public void Show<T>(OnSelectionCallback cb) where T : UnityObject { Show<T>(cb, new string[] { ".prefab" }); }

        /// <summary>
        /// Show the selection wizard.
        /// </summary>

        static public void Show<T>(OnSelectionCallback cb, string[] extensions) where T : UnityObject
        {
            System.Type type = typeof(T);
            string title = (type == typeof(SpriteAtlas) ? "Select an " : "Select a ") + GetName(type);
            ...
            else
            {
                ...
                if (typeof(T) == typeof(Font))
                {
                    ...
                    {
                        UnityObject obj = comp.mObjects[i];
                ...
                System.Array.Sort(comp.mObjects,
                    delegate (UnityObject a, UnityObject b)
                    {
        ...
        void Search()
        {
            ...
            if (mExtensions != null)
            {
                ...
                for (int i = 0; i < paths.Length; ++i)
                {
                    ...
                    UnityObject obj = AssetDatabase.LoadMainAssetAtPath(path);
                ...
                list.Sort(delegate (UnityObject a, UnityObject b) { return a.name.CompareTo(b.name); });
        ...
        void OnGUI()
        {
            ...
            {
                UnityObject sel = null;
                mScroll = GUILayout.BeginScrollView(mScroll);

                foreach (UnityObject o in mObjects)
                    if (DrawObject(o))
                        sel = o;
        ...
        bool DrawObject(UnityObject obj)
        {
