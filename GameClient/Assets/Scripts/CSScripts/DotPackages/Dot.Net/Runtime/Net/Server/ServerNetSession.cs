...
namespace DotEngine.Net.Server
{
    ...
    public class ServerNetSession //: IDispose
    {
        ...
        private void Receive()
        {
            ...
            {
                DebugLog.Error(NetConst.SERVER_LOGGER_TAG, $"ServerNetSession::Receive->Receive message cased a error.message = {e.Message}");
                Disconnect();
            }
        }
        ...
        private void Close()
        {
            ...
            if (socket != null)
            {
                if (socket.Connected)
                {
                    ...
                    {
                        DebugLog.Error(NetConst.SERVER_LOGGER_TAG, $"ServerNetSession::Disconnect->e = {e.Message}");
                    }
                    finally
                    {
                        socket.Close();
        ...
        private void OnHandleSocketEvent(object sender, SocketAsyncEventArgs socketEvent)
        {
            switch(socketEvent.LastOperation)
            {
                ...
                case SocketAsyncOperation.Disconnect:
                    ...
                    break;
                default:
                    DebugLog.Warning(NetConst.SERVER_LOGGER_TAG, $"ClientNetSession::OnHandleSocketEvent->received unkown event.opration = {socketEvent.LastOperation}");
                    break;
            }
        }
        ...
        private void ProcessReceive(SocketAsyncEventArgs socketEvent)
        {
            ...
            DebugLog.Error(NetConst.SERVER_LOGGER_TAG, $"ServerNetSession::ProcessReceive->Receive message error.error = {socketEvent.SocketError}");
            Disconnect();
        }
        ...
        internal void DoLateUpdate()
        {
            ...
            lock (sendingLock)
            {
                if (waitingSendBytes.Count > 0 && !isSending)
                {
                    ...
                    {
                        DebugLog.Error(NetConst.SERVER_LOGGER_TAG, $"ServerNetSession::DoLateUpdate->e = {e.Message}");
                        Disconnect();
                    }
                }
            }
