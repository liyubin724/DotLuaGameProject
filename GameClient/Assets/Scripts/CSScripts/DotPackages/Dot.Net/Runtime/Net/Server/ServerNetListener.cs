...
namespace DotEngine.Net.Server
{
    ...
    public partial class ServerNetListener //: IDispose
    {
        ...
        public void Startup(string ip,int port,int maxCount)
        {
            ...
            DebugLog.Info(NetConst.SERVER_LOGGER_TAG, $"ServerNetListener::Startup->address = {ipAddress.ToString()}");
            ...
            try
            {
                ...
                Thread thread = new Thread(() =>
                {
                    ...
                    {
                        allDone.Reset();
                        DebugLog.Info("ServerNet", "Waiting for a connection...");
            ...
            {
                DebugLog.Info("ServerNet", e.ToString());
            }
        }
        ...
        private void OnMessageReceived(int netID,int messageID,byte[] msgBytes)
        {
            ...
            DebugLog.Error(NetConst.SERVER_LOGGER_TAG, $"ServerNetListener::OnMessageReceived->the handler not found.messageID = {messageID}");
        }
        ...
        public void Dispose()
        {
            ...
            if (socket != null)
            {
                if (socket.Connected)
                {
                    ...
                    {
                        DebugLog.Error(NetConst.SERVER_LOGGER_TAG, $"ServerNetListener::Disconnect->e = {e.Message}");
                    }
                    finally
                    {
                        socket.Close();
        ...
        public void RegisterMessageHandler(string name, IServerNetMessageHandler handler)
        {
            ...
            {
                DebugLog.Error(NetConst.SERVER_LOGGER_TAG, $"ServerNetListener::RegisterMessageHandler->the handler has been added.name={name}");
            }
        }
        ...
        public void UnregisterMessageHandler(string name)
        {
            ...
            {
                DebugLog.Error(NetConst.SERVER_LOGGER_TAG, $"ServerNetListener::UnregisterMessageHandler->The handler not found.name={name}");
            }
        }
