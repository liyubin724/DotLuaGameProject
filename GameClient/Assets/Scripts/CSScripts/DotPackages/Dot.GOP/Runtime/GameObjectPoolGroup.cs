...
namespace DotEngine.GOPool
{
    ...
    public class GameObjectPoolGroup
    {
        ...
        public GameObjectPool GetPool(string uniqueName)
        {
            ...
            {
                DebugLog.Warning(GameObjectPoolConst.LOGGER_NAME, "The pool is not found.name = " + uniqueName);
            }
        ...
        public GameObjectPool CreatePool(string uniqueName, PoolTemplateType templateType, GameObject template)
        {
            if(template == null)
            {
                DebugLog.Error(GameObjectPoolConst.LOGGER_NAME, "GameObjectPoolGroup::CreatePool->Template is Null");
                return null;
            }

            if (poolDic.TryGetValue(uniqueName, out GameObjectPool goPool))
            {
                DebugLog.Warning(GameObjectPoolConst.LOGGER_NAME, "GameObjectPoolGroup::CreatePool->The pool has been created.uniqueName = " + uniqueName);
            }
            else
            {
        ...
        {
            DebugLog.Info(GameObjectPoolConst.LOGGER_NAME, $"GameObjectPoolGroup::DeletePool->Try to delete pool.uniqueName ={uniqueName}");
