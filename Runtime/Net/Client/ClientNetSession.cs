...
namespace DotEngine.Net.Client
{
    ...
    public class ClientNetSession //: IDispose
    {
        ...
        public bool Connect(string ipAddress)
        {
            ...
            {
                DebugLog.Error(NetConst.CLIENT_LOGGER_TAG, "ClientNetSession::Connect->ipAddress is empty");
                return false;
            }
            ...
            {
                DebugLog.Error(NetConst.CLIENT_LOGGER_TAG, $"ClientNetSession::Connect->The lenght of ip is not correct.ipAddress = {ipAddress}");
                return false;
            }

            if (!int.TryParse(splitStrArr[1], out int port) || port<=0)
            {
                DebugLog.Error(NetConst.CLIENT_LOGGER_TAG, $"ClientNetSession::Connect->the port is not a int.port = {splitStrArr[1]}");
                return false;
            }
        ...
        public bool Connect(string ip,int port)
        {
            ...
            {
                DebugLog.Error(NetConst.CLIENT_LOGGER_TAG, $"ClientNetSession::Connect->The ip is empty or the port is not correct.ip = {ip},port={port}");
                return false;
            }
            ...
            if(socket == null)
            {
                try
                {
                    ...
                    DebugLog.Info(NetConst.CLIENT_LOGGER_TAG, $"ClientNetSession::Connect->Begin connecting... (Address = {Address})");

                    socket.ConnectAsync(connectAsyncEvent);
                    return true;
                }catch(Exception e)
                {
                    DebugLog.Error(NetConst.CLIENT_LOGGER_TAG, $"ClientNetSession::Connect->{e.Message}");
            ...
            {
                DebugLog.Error(NetConst.CLIENT_LOGGER_TAG, "ClientNetSession::Connect->The socket has been created.");
            }
            return false;
        }
        ...
        public bool Reconnect()
        {
            if(string.IsNullOrEmpty(ip) || port<=0)
            {
                DebugLog.Error(NetConst.CLIENT_LOGGER_TAG, $"ClientNetSession::Reconnect->the ip is empty,or the port is not correct.");
                return false;
            }
            ...
            DebugLog.Error(NetConst.CLIENT_LOGGER_TAG, $"ClientNetSession::Reconnect->the net is connected or the net is connecting.{State}");
       ...
       private void Receive()
        {
            ...
            {
                DebugLog.Error(NetConst.CLIENT_LOGGER_TAG, $"ClientNetSession::Receive->{e.Message}");
                Disconnect();
            }
        }
        ...
        private void Close()
        {
            DebugLog.Info(NetConst.CLIENT_LOGGER_TAG, "ClientNetSession::Close->Closed Session");
        ...
        private void OnHandleSocketEvent(object sender,SocketAsyncEventArgs socketEvent)
        {
            switch (socketEvent.LastOperation)
            {
                ...
                case SocketAsyncOperation.Disconnect:
                    ...
                    break;
                default:
                    DebugLog.Warning(NetConst.CLIENT_LOGGER_TAG, $"ClientNetSession::OnHandleSocketEvent->received unkown event.opration = {socketEvent.LastOperation}");
                    break;
            }
        }
        ...
        private void ProcessConnect(SocketAsyncEventArgs socketEvent)
        {
            ...
            if(socketEvent.SocketError == SocketError.Success)
            {
                ...
                DebugLog.Info(NetConst.CLIENT_LOGGER_TAG, "ClientNetSession::ProcessConnect->Connect success");
            ...
            {
                DebugLog.Info(NetConst.CLIENT_LOGGER_TAG, $"ClientNetSession::ProcessConnect->Connect failed.error = {socketEvent.SocketError}");
                Close();
                State = ClientNetSessionState.ConnectedFailed;
            }
        }
        ...
        private void ProcessSend(SocketAsyncEventArgs socketEvent)
        {
            if(socketEvent.SocketError == SocketError.Success)
            {
                DebugLog.Info(NetConst.CLIENT_LOGGER_TAG, "ClientNetSession::ProcessSend->Send success");
                lock (sendingLock)
                {
                    isSending = false;
                }
        ...
        private void ProcessReceive(SocketAsyncEventArgs socketEvent)
        {
            if(socketEvent.SocketError == SocketError.Success)
            {
                ...
                {
                    DebugLog.Info(NetConst.CLIENT_LOGGER_TAG, $"ClientNetSession::ProcessReceive->Received message.length = {socketEvent.BytesTransferred}");
                    lock (receiverLock)
                    {
        ...
        internal void DoLateUpdate()
        {
            ...
            lock (sendingLock)
            {
                if (waitingSendBytes.Count > 0 && !isSending)
                {
                    ...
                    {
                        DebugLog.Error(NetConst.CLIENT_LOGGER_TAG, $"ClientNetSession::DoLateUpdate->e = {e.Message}");
                        Disconnect();
                    }
                }
            }
